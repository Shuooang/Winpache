...
CResponse1::CResponse1(SHP<CApiBase> api)//, BOOL bOwner)
...
int CResponse1::ResponseForGet(KSessionInfo& sinfo, stringstream& rpost)
{
	...
	try {
		...
		//SHP<function<int(JObj&, JObj&, int)>>* funcItm = nullptr;
		if(_api->m_mapRFncs.Lookup(fncA, funcItm))//jproc);
		{
			...
			rv = (*funcItm->_func.get())(*sdb, jpa, jsr, iOp);//ok
...
//CStringW resp(rpost.str().c_str());//비동기 람다인 경우는 스택이 사라지므로 람다로 전달할 값을 복사한다.
	...
	try {
		...
		if(jbj.IsObject("params"))///params 없어도 된다.
			jpa = jbj.O("params");

		//string rs = _api->CheckDB();

// 		if(rs.length() > 0)/// 연결 안되어 있으면 창쪽으로 메시지 내보애기 위해 _fncOutput 으로 내보낸다.
// 		{
...
// 		}
		...
		if(_api->m_mapRFncs.Lookup(fncA, funcItm))//jproc);
		{
			...
			/// /////////////////////////////////////////////////////////////////
			rv = (*funcItm->_func)(*sdb, jpa, jsr, iOp);//ok
			/// /////////////////////////////////////////////////////////////////
		...
		else//외부 DLL함수를 디진다
		{
			...
			{
				//SHP<KDatabase> db;
				//if(jpa.SameS("database", L"winpache") && _api->_db._dbLog)
				//	sDsn = _api->_ODBCDSNlog;
					//pdb = _api->_db._dbLog.get();
				//string rs = _api->CheckDbThread(sDsn, db);
				auto su = sdb.use_count();
				if(sdb)
					_api->InitDbLogProc(sdb.get());

				/// /////////////////////////////////////////////////////////////////
				rv = ExDllApiFunc(*sdb, jpa, jsr, 0);
				/// /////////////////////////////////////////////////////////////////
				if(rv == 0)
				{
					//if(jsr.size() > 0)
