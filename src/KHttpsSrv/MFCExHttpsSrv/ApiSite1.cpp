...
string ApiSite1::CheckDB(PWS sDSN)
{
...
// KDatabase* ApiSite1::getDB(JObj& jpa)
// {
// 	auto bMain = jpa.SameS("database", L"winpache");
// 	KDatabase* db = bMain ? _db._dbLog.get() : &_db;
// 	return db;
// }
// 
...
// DB 사용 예제
int ApiSite1::SampleSelectUUID(KDatabase& _db, JObj& jpa, JObj& jrs, int iOp)
{
	try
	{
...
//			limit = L"10";
		sql.Format(L"select * from tuser limit %s", (PWS)limit);
...
int ApiSite1::ClientException(KDatabase& _db, JObj& jpa, JObj& jrs, int iOp)
{
	try
	{
		//AccessRight(jpa, iOp);
