...
typedef function<int(JObj&, JObj&, int)> FuncApi;
typedef shared_ptr<FuncApi> SFuncApi;
...
public:
	...
	CApiBase()
		: _maxFileSize(7000000), _resp(nullptr)
	{
	}
	...
	virtual PAS CheckDB(PWS sDSN, shared_ptr<KDatabase> dbLog);
	virtual PAS CheckDB(PWS sDSN)
	{
		shared_ptr<KDatabase> dbLog;
...
//	void CellToJson(KRecordset& rs, int r, int c, JObj& robj);
	...
	void ResponseFile(PAS pfull, FNCRESP fnc)
	{
		try {
			...
			{
				auto fbuf = shared_ptr<char>(new char[flen+1]);
				*(fbuf.get() + flen) = '\0';
				fl.Read(fbuf.get(), flen);

				fnc(fbuf.get(), flen);//res.MakeGetResponse(fbuf.get());
			}
			else
				fnc(nullptr, flen);
		}
	...
	//CRBMap0<CStringA, shared_ptr<function<int(JObj&,JObj&,int)>>> m_mapRFncs; //FuncApi


	template<typename TFNC>//, typename TRAMBD>
	void AddApi(PAS sFnc, TFNC lambda, int iOp)
	{
		auto sfnc = shared_ptr<FuncApi>(new FuncApi(lambda));//memory leak
	...
	{
		auto fnc = shared_ptr<function<int(JObj&, JObj&, int)>>(new function<int(JObj&, JObj&, int)>(lambda));
	...
	*/
	...
	int SampleApi1(JObj& jpa, JObj& jrs, int iOp);
	int SampleSelect(JObj& jpa, JObj& jrs, int iOp);
	int SampleInsert(JObj& jpa, JObj& jrs, int iOp);
	int SampleUpdate(JObj& jpa, JObj& jrs, int iOp);
	int SampleDelete(JObj& jpa, JObj& jrs, int iOp);

};

#define LogExcepFN(e) LogException(e, nullptr, __FUNCTIONW__, __LINE__)

#define AddLambdaApi(fnc, iOp1) AddApi(#fnc, [&](JObj& jpa, JObj& jrs, int iOp) -> int {	return fnc(jpa, jrs, iOp);	}, iOp1)
