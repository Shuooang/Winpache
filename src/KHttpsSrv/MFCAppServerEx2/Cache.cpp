...
bool CacheBin::GetCacheValue(PAS key, CacheVal& value)
{
	std::scoped_lock locker(_cache_lock);
	CacheVal* cval = nullptr;
	auto it = _cache.find(key);// Lookup(key, cval);
	if (it != _cache.end())
	{
		cval = it->second;
		assert(cval);
...
void CacheBin::PutCacheValue(PAS key, PAS value, INT_PTR len, PAS contentType)
{
	std::scoped_lock locker(_cache_lock);
	...
	//_cache.erase((key))
	CacheVal* cval = new CacheVal(value, len, contentType);
	//auto it = _cache.SetAt(key, cval);
	_cache[key] = cval;
	//auto itf = _fresh.emplace(key, GetTickCount64());//emplace는 갱신은 안된다.
	_fresh[key] = GetTickCount64();
}

// 삭제 하면서 내용은 담아 간다.
bool CacheBin::DeleteCacheValue(PAS key, CacheVal& box)
{
	std::scoped_lock locker(_cache_lock);
