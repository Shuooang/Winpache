...
PAS CApiBase::CheckDB(PWS sDSN, shared_ptr<KDatabase> dbLog)
{
	...
	try 
	{
		...
		if(bDbMustOpen)
		{
			...
			_db.OpenEx(dsn);//_T("DSN=UserInfo"));
		}
		...
		if (dbLog)
		{
			...
			{
				_db._dbLog->TimeCheckReopen();// 연결 안끊기게 시간 지났으면 다시 접속
	...
	string requUtf8, string respUtf8, ULONGLONG elapsed, int retnv, string UA_CPU, string UserAgent)//std::stringstream& rpost)
{
	...
	('%s', '%s', '%s', '%s', '%s', '%s', %d, '%s', %I64u, '%s', '%s', ?, ?)"
			...
			if (!_db._dbLog || !_db._dbLog->IsOpen())
			{
				...
				_db._dbLog->ExecuteSQL(sql, dts, _countof(dts));
			break;
			/*
...
void CApiBase::LogException(CException* e, PWS uuid, PWS func, int line)
{
	//BACKGROUND(3);
	AUTOLOCK(_mxDbLog);//'Packets out of order' 이걸 해도 이 오류가 계속 나네
	bool bFileLog = (!_db._dbLog || !_db._dbLog->IsOpen());
...
// 		return;
	...
	try
	{
		...
		for (int i = 0; i < arq.GetCount(); i++)
		{
			...
			sql.Format(L"insert into t_ldblog (fSiteId, fApp, fClass, fCalled, fTitle, fElpsed, fText, fDetail) values \r\n\
	('%s', 'Winpache', 'query', '%s', '%s', %u, null, ?)"
			...
			if (bFileLog)
			{
				...
				this->_db._dbLog->ExecuteSQL(sql, &dts, 1);
		}
		if (bFileLog)
			flog.Close();
	}
...
bool CApiBase::MapRemoteFunctions()
{
	...
	int iOp1 = 0;
	AddApi("SampleApi1", [&](JObj& jpa, JObj& jrs, int iOp) -> int {	return SampleApi1(jpa, jrs, iOp);	}, iOp1);
...
// DB 미사용 예제
int CApiBase::SampleApi1(JObj& jpa, JObj& jrs, int iOp)
{
	static int si2 = 0;
...
int CApiBase::SampleSelect(JObj& jpa, JObj& jrs, int iOp)
{
	try
	{
		// 1. DB사용 초기화
...
int CApiBase::SampleInsert(JObj& jpa, JObj& jrs, int iOp)
{
	try
	{
...
int CApiBase::SampleUpdate(JObj& jpa, JObj& jrs, int iOp) {
	try
	{
		...
		CString sql;
		sql.Format(L"update t_uuid set fuuid = '%s' where fsrcuuid = '%s'"
			, jpa.S("fuuid"), jpa.S("fsrcuuid"));
		_db.ExecuteSQL(sql);
		jrs("Return") = L"OK";
	}CATCH_DBTR;
	return 0;
...
int CApiBase::SampleDelete(JObj& jpa, JObj& jrs, int iOp) {
