...
{
public:
	CResponse1(shared_ptr<CApiBase> api = NULL);//, BOOL bOwner = TRUE);
	~CResponse1();

	/// 실제 원격 함수가 집합체 객체의 포인터
	/// base type share_ptr에 할당 할때는 derived type객체 인스턴스를 가져 온다.
	shared_ptr<CApiBase> _api; //  = make_shared< ApiSite1 >();
	...
	int ResponseForPost(KSessionInfo& sinfo, shared_ptr<KBinData> body, stringstream& rpost);
