...
public:
	...
	//int _mapMode{ MM_TEXT };
	void InitGC(HDC hDC)
	{
		_hDC = hDC;
#ifdef _ZOOM
		SetMapMode(_hDC, MM_ANISOTROPIC);// MM_HIMETRIC);
		CDC dc;
		dc.FromHandle(_hDC);
		dc.SetMapMode(MM_ANISOTROPIC);
		dc.SetWindowOrg(0, 0);
		dc.SetWindowExt(CSize(200,200));
	...
	double _zoom{ 1.0 };
	Gdiplus::Rect GetMapRc(int l, int t, int w, int h)
	{
		CRect rc(CPoint(l, t), CSize(w, h));
		return GetMapRc(rc);
	}
	Gdiplus::Rect GetMapRc(CRect rc)
	{
		Gdiplus::Rect grc; //(_rcc.left, _rcc.top, _rcc.Width(), _rcc.Height());
#ifdef _ZOOM
		int mm = GetMapMode(_hDC);// , MM_HIMETRIC);
		bool br = mm == MM_ANISOTROPIC;
		double zm = (br ? _zoom : 1);
		grc.X = (INT)(zm * rc.left);
		grc.Y = (INT)(rc.left * zm) * (br ? -1. : 1.);
		grc.Width = (INT)(rc.Width() * zm);
		grc.Height = (INT)(rc.Height() * zm);
#else
		grc.X = rc.left;
		grc.Y = rc.left;
		grc.Width = rc.Width();
		grc.Height = rc.Height();
#endif
		return grc;
	}
	...
	KStrMap<wstring, int> _requestedIDX;

	void AddOrder(int col, int row);
