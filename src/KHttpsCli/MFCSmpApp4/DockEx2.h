...
{
	DECLARE_DYNCREATE(DockEx2)

public:
	DockEx2(UINT nID = 0, CPaneForm* ppn = NULL);           // 동적 만들기에 사용되는 protected 생성자입니다.
	virtual ~DockEx2();
	static DockEx2* s_me;

public:
#ifdef AFX_DESIGN_TIME
	enum { IDD = IDD_DockEx2	};
#endif
#ifdef _DEBUG
...
class DockEx2Pane : public CPaneForm
{
	DECLARE_DYNCREATE(DockEx2Pane)
public:
	//CreateDockingWin 에서 채워진다. 이 pane이 아니고 child formview의 ID
	virtual DockClientBase* _CreateForm() { ASSERT(_nID != 0);  return new DockEx2(_nID, this); }
};
