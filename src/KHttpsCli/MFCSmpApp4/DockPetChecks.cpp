...
IMPLEMENT_DYNCREATE(DockPetChecksPane, CPaneForm)

// DockPetChecks

IMPLEMENT_DYNCREATE(DockPetChecks, DockClientBase)
DockPetChecks* DockPetChecks::s_me = NULL;

DockPetChecks::DockPetChecks(UINT nID, CPaneForm* ppn)
	: DockClientBase(nID, ppn) //DockPetChecks::IDD IDD_DockPetChecks
{
	s_me = this;
}

DockPetChecks::~DockPetChecks()
{
}

void DockPetChecks::DoDataExchange(CDataExchange* pDX)
{
	__super::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_MFCPROPERTYGRID1, _cProperty);
}

BEGIN_MESSAGE_MAP(DockPetChecks, DockClientBase)
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_BUTTON1, &DockPetChecks::OnBnClickedButton1)
	ON_WM_SETFOCUS()
	ON_WM_SETTINGCHANGE()
	ON_NOTIFY(NM_THEMECHANGED, IDC_MFCPROPERTYGRID1, &DockPetChecks::OnNMThemeChangedMfcpropertygrid1)
	ON_STN_CLICKED(IDC_MFCPROPERTYGRID1, &DockPetChecks::OnStnClickedMfcpropertygrid1)
	ON_STN_DBLCLK(IDC_MFCPROPERTYGRID1, &DockPetChecks::OnStnDblclickMfcpropertygrid1)
END_MESSAGE_MAP()
...
#ifdef _DEBUG
void DockPetChecks::AssertValid() const
{
	__super::AssertValid();
}

#ifndef _WIN32_WCE
void DockPetChecks::Dump(CDumpContext& dc) const
{
	__super::Dump(dc);
}
#endif
...
// DockPetChecks 메시지 처리기입니다.
// CBranchVu1 메시지 처리기입니다.
void DockPetChecks::OnInitialUpdate()
{
	if(m_bInited)
		return;
...
void DockPetChecks::AdjustLayout()
{
...
void DockPetChecks::OnSize(UINT nType, int cx, int cy)
{
...
void DockPetChecks::OnSetFocus(CWnd* pOldWnd)
{
	DockClientBase::OnSetFocus(pOldWnd);
	_cProperty.SetFocus();
}

void DockPetChecks::OnBnClickedButton1()
{
	//auto app = (CMfcSmpApp1App*)AfxGetApp();
	auto mfm = (CMainFrame*)AfxGetMainWnd();
	auto d1 = mfm->Dock(IDD_DockMyPetChecks);
	if(d1)
		AfxMessageBox(L"DockPetChecks ok");
}



void DockPetChecks::InitPropList(ShJObj sjo)
{
	//SetPropListFont();
...
void DockPetChecks::OnNMThemeChangedMfcpropertygrid1(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void DockPetChecks::OnStnClickedMfcpropertygrid1()
{
	TRACE("%s \n", __FUNCTION__);
}


void DockPetChecks::OnStnDblclickMfcpropertygrid1()
{
	TRACE("%s \n", __FUNCTION__);
}
