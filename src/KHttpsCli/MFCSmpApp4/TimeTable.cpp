...
void TimeTable::DrawTimeTable()
{
	...
	*/
	...
	if(!_bRedraw)
	{
		Gdiplus::Rect rc1(_rcc.left, _rcc.top, _rcc.Width(), _rcc.Height());
		_gc->FillRectangle(&brush, rc1);// .left, rc.top, rc.Width(), rc.Height());
		_gc->Flush();
		return;
	}
	CRect rc = Rc();
	Gdiplus::Rect rc1(rc.left, rc.top, rc.Width() + 1, rc.Height() + 1);
	_gc->FillRectangle(&brush, rc1);// .left, rc.top, rc.Width(), rc.Height());
...
void TimeTable::DrawOneBox(TBox* box, float wth)
{
	...
	Gdiplus::Rect rc1(box->_rc.left, box->_rc.top, box->_rc.Width() - 1, box->_rc.Height() - 1);
	Color cl;
...
void TimeTable::DrawConer()
{
	...
	CRect rc = getRcConer();
	Gdiplus::Rect rc1(rc.left, rc.top, rc.Width()-1, rc.Height());
...
void TimeTable::DrawCells()
{
	...
	for(int r = 0; r < _nRows; r++)
	{
		for(int c = 0; c < _nCols; c++)
		{
			...
			int adj = r == 0 ? 1 : 0;
			Gdiplus::Rect rc1(rc.left, rc.top+ adj, rc.Width(), rc.Height()- adj);
...
void TimeTable::AddOrder(int col, int row, ShJObj sjo)
{
	ShJObj vch = _col[col];
	TBox* ptx = new TBox;
	TBox& tx = *ptx;
	...
	CString fOrderID;
	if(sjo)
		sjo->LenS("fOrderID", fOrderID);
	if(fOrderID.GetLength() == 0)
#ifdef DEBUG
		fOrderID = KwGetFormattedGuid(true, L"order", 8);
#else
		fOrderID = KwGetFormattedGuid();
#endif
	jocc("fSchedOrgID") = fOrderID;
	...
	_requestedIDX[(PWS)fOrderID] = newIDX;
