...
{
	DECLARE_DYNCREATE(DockPetChecks)

public:
	DockPetChecks(UINT nID = 0, CPaneForm* ppn = NULL);           // 동적 만들기에 사용되는 protected 생성자입니다.
	virtual ~DockPetChecks();
	static DockPetChecks* s_me;

public:
#ifdef AFX_DESIGN_TIME
	enum { IDD = IDD_DockPetChecks };
#endif
#ifdef _DEBUG
...
class DockPetChecksPane : public CPaneForm
{
	DECLARE_DYNCREATE(DockPetChecksPane)
public:
	//CreateDockingWin 에서 채워진다. 이 pane이 아니고 child formview의 ID
	virtual DockClientBase* _CreateForm() { ASSERT(_nID != 0);  return new DockPetChecks(_nID, this); }
};
