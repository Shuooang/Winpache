...
#pragma comment(lib, "Userenv.lib") //2>CppCommon3.lib(path.obj) : error LNK2019: __imp_GetUserProfileDirectoryW 외부 기호(참조 위치: "public: static class CppCommon::Path __cdecl CppCommon::Path::home(void)" (?home@Path@CppCommon@@SA?AV12@XZ) 함수)에서 확인하지 못했습니다.
Path Path::home()
{
#if defined(unix) || defined(__unix) || defined(__unix__) || defined(__APPLE__)
    std::vector<char> buffer(PATH_MAX);
