      ...
      _option_reuse_port(false)
{
    ...
    switch (protocol)
    {
        case InternetProtocol::IPv4:
            _endpoint = BAsio::ip::udp::endpoint(BAsio::ip::udp::v4(), (unsigned short)port);
            break;
        case InternetProtocol::IPv6:
            _endpoint = BAsio::ip::udp::endpoint(BAsio::ip::udp::v6(), (unsigned short)port);
            break;
    }
}
      ...
      _option_reuse_port(false)
{
    ...
    // Prepare endpoint
    _endpoint = BAsio::ip::udp::endpoint(BAsio::ip::make_address(address), (unsigned short)port);
}

UDPServer::UDPServer(const std::shared_ptr<Service>& service, const BAsio::ip::udp::endpoint& endpoint)
    : _id(CppCommon::UUID::Sequential()),
      _service(service),
      _io_service(_service->GetAsioService()),
      _strand(*_io_service),
...
{
    _multicast_endpoint = BAsio::ip::udp::endpoint(BAsio::ip::make_address(multicast_address), (unsigned short)multicast_port);
    return Start();
}

bool UDPServer::Start(const BAsio::ip::udp::endpoint& multicast_endpoint)
{
...
size_t UDPServer::Send(const BAsio::ip::udp::endpoint& endpoint, const void* buffer, size_t size)
{
    if (!IsStarted())
        return 0;
...
size_t UDPServer::Send(const BAsio::ip::udp::endpoint& endpoint, const void* buffer, size_t size, const CppCommon::Timespan& timeout)
{
    if (!IsStarted())
        return 0;
...
bool UDPServer::SendAsync(const BAsio::ip::udp::endpoint& endpoint, const void* buffer, size_t size)
{
    if (_sending)
        return false;
...
size_t UDPServer::Receive(BAsio::ip::udp::endpoint& endpoint, void* buffer, size_t size)
{
    if (!IsStarted())
        return 0;
...
std::string UDPServer::Receive(BAsio::ip::udp::endpoint& endpoint, size_t size)
{
...
size_t UDPServer::Receive(BAsio::ip::udp::endpoint& endpoint, void* buffer, size_t size, const CppCommon::Timespan& timeout)
{
    if (!IsStarted())
        return 0;
...
std::string UDPServer::Receive(BAsio::ip::udp::endpoint& endpoint, size_t size, const CppCommon::Timespan& timeout)
{
    std::string text(size, 0);
...
void UDPServer::SendError(TErrCode ec)
{
