      ...
      _option_reuse_port(false)
{
    ...
    switch (protocol)
    {
        case InternetProtocol::IPv4:
            _endpoint = BAsio::ip::tcp::endpoint(BAsio::ip::tcp::v4(), (unsigned short)port);
            break;
        case InternetProtocol::IPv6:
            _endpoint = BAsio::ip::tcp::endpoint(BAsio::ip::tcp::v6(), (unsigned short)port);
            break;
    }
}
      ...
      _option_reuse_port(false)
{
    ...
    // Prepare endpoint
    _endpoint = BAsio::ip::tcp::endpoint(BAsio::ip::make_address(address), (unsigned short)port);
}

SSLServer::SSLServer(const std::shared_ptr<Service>& service, const std::shared_ptr<SSLContext>& context, const BAsio::ip::tcp::endpoint& endpoint)
    : _id(CppCommon::UUID::Sequential()),
      _service(service),
      _io_service(_service->GetAsioService()),
      _strand(*_io_service),
...
bool SSLServer::Start()
{
    ...
    auto start_handler = [this, self]()
    {
        ...
        _acceptor = BAsio::ip::tcp::acceptor(*_io_service);
        _acceptor.open(_endpoint.protocol());
        if (option_reuse_address())
            _acceptor.set_option(BAsio::ip::tcp::acceptor::reuse_address(true));
...
bool SSLServer::Stop()
{
    assert(IsStarted() && "SSL server is not started!");//kdw : exception port가 방화벽에 안열렸나?
    if (!IsStarted())
        return false;
...
void SSLServer::Accept()
{
    ...
    auto accept_handler = make_alloc_handler(_acceptor_storage, [this, self]()
    {
        ...
        auto async_accept_handler = make_alloc_handler(_acceptor_storage, [this, self](TErrCode ec)
        {
            if (!ec)
            {
                RegisterSession();
...
void SSLServer::SendError(TErrCode ec)
{
