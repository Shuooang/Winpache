...
#include <string_view>
#include <vector>
//#include <boost/test/execution_monitor.hpp> //kdw: boost //?코드변경 하려다 만다.
...
public:
    ...
    */
    explicit Service(const std::shared_ptr<boost::asio::io_service>& service, bool strands = false);//?코드변경 1 asio:: => boost::asio 로 바꿔서 include path에 맞춘다.
    ...
    */
    virtual std::shared_ptr<boost::asio::io_service>& GetAsioService() noexcept
    ...
    */
	/*kdw: 오류가 나서 맊아놓음. //?코드변경 2.2 오류가 나서 맊아놓음.
	...
	*/
    ...
    /*!
        ...
        Method takes a handler to dispatch as a parameter and returns async result of the handler.
    */
	/*kdw: 오류가 나서 맊아놓음. //?코드변경 2.3 오류가 나서 맊아놓음.
	template <typename CompletionHandler>
...
private:
    // Asio IO services
    std::vector<std::shared_ptr<boost::asio::io_service>> _services;
    ...
    //! Service thread
    static void ServiceThread(const std::shared_ptr<Service>& service, const std::shared_ptr<boost::asio::io_service>& io_service);

    //! Send error notification
    void SendError(TErrCode ec);//?코드변경 3 std::error_code 가 오류 나서 boost::system::error_code  로 바꿔
};
